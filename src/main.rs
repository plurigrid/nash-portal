...
#[derive(Clone)]
struct Candle { o: f64, h: f64, l: f64, c: f64 }
...
impl App {
    fn new() -> Self {
        Self { data: TokenData::default(), candles: Vec::new(), timeframe: Timeframe::Min5,
               ticker_offset: 0, tick: 0, status: "Loading...".into() }
    }
...
async fn fetch_candles(tf: Timeframe) -> Option<Vec<Candle>> {
    ...
    let resp = gloo_net::http::Request::get(&url).send().await.ok()?;
    let v: serde_json::Value = resp.json().await.ok()?;
    let list = v["data"]["attributes"]["ohlcv_list"].as_array()?;
...
fn do_refresh(state: &Rc<RefCell<App>>) {
    ...
    spawn_local(async move {
        if let Some(data) = fetch_price().await {
            s.borrow_mut().data = data;
        }
        let tf = s.borrow().timeframe;
        if let Some(c) = fetch_candles(tf).await {
...
fn render_candles(f: &mut Frame, area: Rect, candles: &[Candle], tf: Timeframe) {
    ...
    let norm = |v: f64| -> usize {
        ...
        ((1.0 - pct) * (h as f64 - 1.0)).round().max(0.0).min((h - 1) as f64) as usize
    };
    ...
    for (i, candle) in visible.iter().enumerate() {
        ...
        for row in otop..=obot {
            if row < h {
                grid[row][col] = ('┃', color);
                if col + 1 < w { grid[row][col+1] = ('┃', color); }
            }
        }
    ...
    if w > 12 {
        ...
        for (ci, ch) in hi_s.chars().enumerate() { if w.saturating_sub(11) + ci < w { grid[0][w-11+ci] = (ch, Color::Yellow); } }
        if h > 1 { for (ci, ch) in lo_s.chars().enumerate() { if w.saturating_sub(11) + ci < w { grid[h-1][w-11+ci] = (ch, Color::Yellow); } } }
        if h > 4 { for (ci, ch) in mid_s.chars().enumerate() { if w.saturating_sub(11) + ci < w { grid[h/2][w-11+ci] = (ch, Color::DarkGray); } } }
    }
    ...
    let pct = if range > 0.0 { ((last.c - all_low) / range * 100.0) as i32 } else { 50 };
    let title = format!(" {} │ {} candles │ ${:.8} │ {}% ", tf.label(), visible.len(), last.c, pct);
...
fn draw(f: &mut Frame, app: &App) {
    let outer = Layout::default().direction(Direction::Vertical).constraints([
        Constraint::Length(1),  Constraint::Length(3),  Constraint::Length(5),
        Constraint::Min(12),   Constraint::Length(3),  Constraint::Length(1),
    ]).split(f.area());
    ...
    if !chars.is_empty() {
        ...
        f.render_widget(Paragraph::new(Span::styled(vis, Style::default().fg(tc).add_modifier(Modifier::BOLD))), outer[0]);
    }

    // Price bar
    let d = &app.data;
    let pc = if d.change_5m > 0.0 {Color::Green} else if d.change_5m < 0.0 {Color::Red} else {Color::White};
    f.render_widget(Paragraph::new(Line::from(vec![
        Span::styled(" NASH ", Style::default().fg(Color::White).bg(Color::Magenta).add_modifier(Modifier::BOLD)),
        Span::styled(format!("  ${:.8}", d.price_usd), Style::default().fg(pc).add_modifier(Modifier::BOLD)),
        Span::styled(format!("  {:.6} SOL", d.price_sol), Style::default().fg(Color::Cyan)),
        Span::styled(format!("  MCap ${:.0}K  FDV ${:.0}K", d.market_cap/1000.0, d.fdv/1000.0), Style::default().fg(Color::Yellow)),
...
fn main() -> io::Result<()> {
    ...
    let _ticker = Interval::new(150, move || {
        ticker_state.borrow_mut().ticker_offset += 1;
    });

    // Data poll — 10s
    let poll_state = Rc::clone(&state);
    let _poll = Interval::new(10_000, move || {
        do_refresh(&poll_state);
    });
